			...
			AIController:function(){
				...
				function Action02(){
					...
					function StoneFalling(){
						...
						var FallingStone= document.getElementsByClassName("EnemyStoneForJs");
						var timer11 = setInterval(function(){
							if (parseInt(getStyle(FallingStone[0],"top"))<423) {
								for (var i = 0; i < 3; i++) {
									FallingStone[i].style.top=parseInt(getStyle(FallingStone[i],"top"))+20+"px";
									if (collision(Obj.Player,FallingStone[i])) {
										Mc.GetHurt();
									}									
								};	
							}
								...
								for (var i = 0; i < 3; i++) {
									//FallingStone[i].style.display="none";
